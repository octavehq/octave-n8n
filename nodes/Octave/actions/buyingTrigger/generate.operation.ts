...
const properties: INodeProperties[] = [
	{
		...
		hint: 'If not provided, the primary company attached to the Workspace will be used.',
	},
	{
		displayName: 'Buying Trigger Generation Requests',
		name: 'buyingTriggers',
		type: 'fixedCollection',
		placeholder: 'Add Buying Trigger',
		typeOptions: { multipleValues: true },
		default: {},
		description: 'Each request generates one buying trigger',
		options: [
			{
				displayName: 'Buying Trigger',
				name: 'buyingTrigger',
				values: [
					{
						displayName: 'Name',
	...
	},
	{
		...
		type: 'string',
		default: '',
		description: 'Brand voice OId to apply to generated buying triggers (optional)',
	},
	{
		...
		default: 'NONE',
		description: 'Strategy for linking generated buying triggers to products',
	},
	{
...
export async function execute(this: IExecuteFunctions, itemIndex: number): Promise<INodeExecutionData[][] | null> {
	...
	const triggersRaw = this.getNodeParameter('buyingTriggers', itemIndex) as { buyingTrigger?: Array<{ name?: string; sources: { source?: Array<{ type: string; value: string }> } }> };
	const triggers = (triggersRaw?.buyingTrigger || []).map(t => ({
		name: t.name,
		sources: t.sources?.source || [],
	}));
	if (triggers.length === 0) {
		throw new NodeOperationError(this.getNode(), 'At least one buying trigger generation request is required.', { itemIndex });
	}
	body.buyingTriggers = triggers;
	...
	const responseData = await octaveApiRequest.call(this, 'POST', '/api/v2/buying-trigger/generate', body);
