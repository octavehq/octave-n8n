...
const properties: INodeProperties[] = [
    ...
    },
    {
        ...
        type: 'json',
        default: '{}',
        description: 'Additional JSON instructions for personalization (optional)',
    },
    {
...
export async function execute(this: IExecuteFunctions, itemIndex: number): Promise<INodeExecutionData[][] | null> {
    ...
    // Spread additional inputs
    const finalBody = { ...body, ...additionalInputs };

    Object.keys(finalBody).forEach(key => (finalBody[key] === undefined) && delete finalBody[key]);

    const responseDataOuter = await octaveApiRequest.call(this, 'POST', '/api/v2/agents/personalize-template/run', finalBody);
