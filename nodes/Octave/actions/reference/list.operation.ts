...
export async function execute(this: IExecuteFunctions, itemIndex: number): Promise<INodeExecutionData[][] | null> {
    ...
    if (returnAll) {
        ...
        responseDataInner = await octaveApiRequestListAll.call(this, 'GET', '/api/v2/reference/list', qs);
    } else {
        qs.limit = this.getNodeParameter('limit', itemIndex, 50) as number;
        qs.offset = this.getNodeParameter('offset', itemIndex, 0) as number;
        const responseDataOuter = await octaveApiRequest.call(this, 'GET', '/api/v2/reference/list', {}, qs);
